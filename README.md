...
Отказ от `Geo-based DNS` обоснован тем, что сервис довольно высоконагруженный и существует риск атаки, которая будет сконцентрирована в одном дата-центре из-за того, что его выбор основан на `GeoIP-базе`. Как уже было сказано, задержка (Latency) для данного сервиса не важна, поэтому `Latency-based DNS` здесь не имеет смысл. `Использование CDN в качестве хаба для обеспечения более стабильного соединения` не целесообразно, т.к. нет необходимости в стабильном и качественном подключении для создания событий календаря.
