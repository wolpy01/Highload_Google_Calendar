...
| Пользователь (профиль) | Календарь       | Событие в календаре | Уведомление                      | Ответ на уведомление | Напонимание                      |
| ---------------------- | --------------- | ------------------- | -------------------------------- | -------------------- | -------------------------------- |
| email                  | Название        | Дата создания       | Календарь (ссылка)               | Ответ пользователя   | Календарь (ссылка)               |
| Хэшированный пароль    | Описание        | Дата обновления     | Владелец (ссылка)                |                      | Владелец (ссылка)                |
...
| **Общий размер**       | 5475 \* (12 + 12 + 12 + 12 + 372 + 3072 + 8 + 8 + 1 + 12) = 5475 \* 3533 = 19343175 B < `19.35 MiB` (принимаем 19.35 MiB) |
...
| Календарь           | 8 B (ссылка)                                                                                |
| Пользователь        | 8 B (ссылка)                                                                                |
| email приглашённого | 255 B                                                                                       |
| Событие             | 8 B (ссылка)                                                                                |
| Ответ пользователя  | 1 B (uint_8t)                                                                               |
| **Общий размер**    | 200 \* (8 + 8 + 255 + 8 + 1) = 200 \* 280 = 56000 B < `56 KiB` (принимаем 56 KiB) |
