...
Выбор `Geo-based DNS` обоснован тем, что пользователя будет направлять в ближайший по расположению дата-центр, который уже был заранее определён составлением `Geo-map`. `BGP Anycast` используется по схожей причине, т.к. позволяет распределить нагрузку между отдельными регионами с помощью настройки anycast-сетей.
...
Как уже было сказано, задержка (Latency) для данного сервиса не важен,, поэтому `Latency-based DNS` здесь не имеет смысл. `Использование CDN в качестве хаба для обеспечения более стабильного соединения` не нужен, т.к. нет необходимости в стабильном и качественном подключении для создания событий календаря
